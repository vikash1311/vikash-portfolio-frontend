...
const ROLES = ['Full Stack Developer', 'AI Enthusiast', 'Spring Boot Engineer', 'React.js Builder', 'AI Researcher'];
...
export default function Hero() {
  ...
  const statsRef = useRef(null);
  const [counted, setCounted] = useState(false);
  const [counts, setCounts] = useState({ apps: 0, users: 0, papers: 0, cgpa: 0 });
  ...
  function animateCounters() {
    const targets = { apps: 5, users: 10, papers: 3, cgpa: 846 };
    let start = null;
    const step = (ts) => {
      if (!start) start = ts;
      const progress = Math.min((ts - start) / 1800, 1);
  ...
  return (
    <section id="about" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '8rem 4rem 4rem', position: 'relative', overflow: 'visible' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center', width: '100%', maxWidth: 1200, margin: '0 auto' }} className="hero-grid">
        ...
        <div>
          {/* Status badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 100, padding: '0.35rem 0.9rem', marginBottom: '1.5rem' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#10B981', animation: 'pulse 2s infinite', display: 'inline-block' }} />
            <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '0.72rem', color: '#065F46', fontWeight: 600 }}>Open to full-time & freelance roles</span>
          </div>

          {/* Name — consistent Syne font, no clash */}
          <h1 style={{ fontSize: 'clamp(3rem,6vw,5.5rem)', fontWeight: 800, lineHeight: 1.0, letterSpacing: '-0.03em', marginBottom: '0.3rem', fontFamily: "'Syne',sans-serif" }}>
            Vikash<br />
            <span style={{ color: '#1A6CFF' }}>Gautam</span>
          </h1>

          {/* Typewriter role */}
          <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '1rem', color: '#1A6CFF', marginBottom: '1.4rem', height: '1.5rem', display: 'flex', alignItems: 'center', gap: 4 }}>
            <span>{role}</span>
            <span style={{ borderRight: '2px solid #1A6CFF', height: '1.1em', animation: 'blink 0.8s infinite' }} />
          </div>

          {/* Summary — from resume */}
          <p style={{ fontSize: '1rem', color: 'var(--muted,#6B7280)', lineHeight: 1.85, marginBottom: '2rem', maxWidth: 490 }}>
            Full Stack Developer with production internship experience delivering <strong style={{ color: 'var(--black,#0A0A0A)' }}>5+ live applications</strong> across government and private sector clients. Optimized a government-scale system — cutting login time from <strong style={{ color: 'var(--black,#0A0A0A)' }}>30–60s to under 5s</strong> and data load from <strong style={{ color: 'var(--black,#0A0A0A)' }}>2+ min to under 10s</strong>. Combines solid backend and frontend engineering with an <strong style={{ color: 'var(--black,#0A0A0A)' }}>AI research background</strong> — 3 published papers and a Government of India copyright.
          </p>

          {/* Stats */}
          <div ref={statsRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '2.5rem', padding: '1.2rem', background: 'var(--card,#fff)', borderRadius: 16, border: '1px solid var(--border,rgba(10,10,10,0.08))' }}>
            {[
              { num: counts.apps + '+', label: 'Live apps' },
              { num: counts.users + 'k+', label: 'Users served' },
              { num: counts.papers, label: 'Papers' },
              { num: (counts.cgpa / 100).toFixed(2), label: 'CGPA' },
            ].map(s => (
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#1A6CFF', fontFamily: "'DM Mono',monospace" }}>{s.num}</div>
                <div style={{ fontSize: '0.65rem', color: 'var(--muted,#6B7280)', fontFamily: "'DM Mono',monospace", marginTop: 2 }}>{s.label}</div>
              </div>
            ))}
          </div>
          ...
          {/* CTAs */}
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            <a href="#projects" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: '#1A6CFF', color: '#fff', padding: '0.85rem 2rem', borderRadius: 100, fontSize: '0.88rem', fontWeight: 600, textDecoration: 'none', transition: 'transform 0.2s', cursor: 'none' }}
              onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'}
              onMouseLeave={e => e.currentTarget.style.transform = ''}>
              View Projects →
            </a>
            <a href="#resume" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'transparent', color: 'var(--black,#0A0A0A)', padding: '0.85rem 2rem', borderRadius: 100, border: '1.5px solid var(--border,rgba(10,10,10,0.2))', fontSize: '0.88rem', fontWeight: 600, textDecoration: 'none', transition: 'transform 0.2s, border-color 0.2s, color 0.2s', cursor: 'none' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.borderColor = '#1A6CFF'; e.currentTarget.style.color = '#1A6CFF'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.borderColor = 'var(--border,rgba(10,10,10,0.2))'; e.currentTarget.style.color = 'var(--black,#0A0A0A)'; }}>
              View Resume
            </a>
          </div>
        </div>
        ...
        <div className="hero-visual" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.7rem' }}>
            {[
              ...
              <div key={i} style={{ background: 'var(--card,#fff)', border: '1px solid var(--border,rgba(10,10,10,0.08))', borderRadius: 100, padding: '0.45rem 0.9rem', fontSize: '0.75rem', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '0.4rem', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', animation: `float2 ${4 + i}s ease-in-out infinite ${i * 0.5}s`, color: 'var(--black,#0A0A0A)' }}>
                {b.dot && <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#00D4AA', animation: 'pulse 2s infinite', flexShrink: 0 }} />}
                {b.text}
              </div>
            ))}
